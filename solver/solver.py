...
class Flow(nn.Module):
    def __init__(self, block_number, feature_dim):
        super().__init__()
        self.model = nn.ModuleList([block(feature_dim) for _ in range(block_number)])
...
class Solver(BaseSolver):
    def __init__(self, net, dataloader, bn_domain_map={}, resume=None, **kwargs):
        ...
        self.alignnet = to_cuda(Flow(block_number=self.opt.HYPER.BLOCK_NUMBER, feature_dim=2048))
        
        params=[{'params': self.alignnet.parameters(), 'lr_mult': 10.0}]

        if self.opt.TRAIN.OPTIMIZER == "Adam":
            self.opt_align = optim.Adam(params,
			lr=self.base_lr, betas=[self.opt.ADAM.BETA1, self.opt.ADAM.BETA2], 
			weight_decay=self.opt.TRAIN.WEIGHT_DECAY)
        elif self.opt.TRAIN.OPTIMIZER == "SGD":
            self.opt_align = optim.SGD(params,
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=self.opt.TRAIN.WEIGHT_DECAY)

    def update_lr_net(self):
        iters = self.iters
        num = self.num_layers
        if self.opt.TRAIN.LR_SCHEDULE == 'exp':
            for i in range(num):
                solver_utils.adjust_learning_rate_exp(self.base_lr, 
			            self.opt_align, iters, 
                        decay_rate=self.opt.EXP.LR_DECAY_RATE,
			            decay_step=self.opt.EXP.LR_DECAY_STEP)

        elif self.opt.TRAIN.LR_SCHEDULE == 'inv':
            for i in range(num):
                solver_utils.adjust_learning_rate_inv(self.base_lr,self.opt_align, 
    ...
    def update_network(self, filtered_classes):
        ...
        while not stop:
            ...
            source_logits = self.C(source_feature)

            source_feature_t = self.alignnet(source_feature.detach(), reverse=True).detach()
            source_feature_t_logits = self.C(source_feature_t)
            
            # compute the cross-entropy loss
            ce_loss = self.CELoss(source_logits, source_gt) + self.CELoss(source_feature_t_logits, source_gt)
            ...
            target_feature = self.F(target_data)
            target_logits = self.C(target_feature)

            target_feature_s = self.alignnet(target_feature.detach()).detach()
            target_feature_s_logits = self.C(target_feature_s)

            con_loss = 0.1*self.Con_loss(target_logits, target_feature_s_logits)
            se_loss = ce_loss + con_loss
            se_loss.backward()
            ...
            if len(filtered_classes) > 0:
                ...
                probs_source = nn.Softmax(dim=1)(self.C(feats_source))
                self.net.module.set_bn_domain(self.bn_domain_map[self.target_name])
                feats_target = self.F(target_cls_concat)
                probs_target = nn.Softmax(dim=1)(self.C(feats_target))

                # prepare the features
                # feats_toalign_S = self.prepare_feats(feats_source)
                # feats_toalign_T = self.prepare_feats(feats_target)
                
                feats_source_t = [self.alignnet(feats_source.detach(), reverse=True)]
                feats_target_s = [self.alignnet(feats_target.detach())]
                feats_source = [feats_source.detach()]
                feats_target = [feats_target.detach()]
                probs_source = [probs_source]
                probs_target = [probs_target]
                cdd_loss_f = self.cdd.forward(feats_source, feats_target_s, source_nums_cls, target_nums_cls)[self.discrepancy_key] + self.cdd.forward(feats_target, feats_source_t, source_nums_cls, target_nums_cls)[self.discrepancy_key]
                cdd_loss_p = self.cdd.forward(probs_source, probs_target, source_nums_cls, target_nums_cls)[self.discrepancy_key]
                cdd_loss = cdd_loss_f+cdd_loss_p
                cdd_loss *= self.opt.HYPER.CDD_WEIGHT
                cdd_loss.backward()
